...
public interface FtBrainRepository extends JpaRepository<FtBrain,String> {
    List<FtBrain> findByGeneid(String geneid);
    List<FtBrain> findBySymbolLike(String symbol);

    @Query(value ="select distinct u.max_name from ft_brain u order by u.max_name asc,length (u.max_name) asc, u.max_name asc", nativeQuery = true)
